FKGExpCal = 
	ExpCal: (CalData) !->
		...
		if CalData.InpPrompt is 0
			LevelMax = 0
		else if CalData.InpPrompt is 1
			LevelMax = 10
		else if CalData.InpPrompt is 2
			LevelMax = 20

		TableInd = 0
		if CalData.InpRarity is 2 or CalData.InpRarity is 3
			LevelMax = LevelMax + 50
		else if CalData.InpRarity is 4
			LevelMax = LevelMax + 50
			TableInd = 1
		else if CalData.InpRarity is 5
			LevelMax = LevelMax + 60
			TableInd = 2
		else if CalData.InpRarity is 6
			LevelMax = LevelMax + 60
			TableInd = 3
